...
document.getElementById('form-sessao').addEventListener('submit', function (e) {
  ...
  // Verificação de duplicata (apenas em novo cadastro)
  const sessaoDuplicada = sessoes.find(s =>
    s.filmeIndex === sessao.filmeIndex &&
    s.salaIndex === sessao.salaIndex &&
    s.dataHora === sessao.dataHora &&
    s.idioma === sessao.idioma &&
    s.formato === sessao.formato &&
    s.preco === sessao.preco
  );

  if (!indexEdicao && sessaoDuplicada) {
    alert('Sessão já cadastrada com os mesmos dados!');
    return;
  }
